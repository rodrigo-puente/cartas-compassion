...
function generateHeader(doc, path, vineta){
  ...
  vineta.tipo_carta.forEach((i, idx) => {
    const x = 420 + (10 * idx);
    const image = doc.openImage(`${path}/src/assets/cartas/${i}.png`);
  ...
  const qr = doc.openImage(`${path}/qrcode.png`);
  doc.image(qr, 150, 65, {
    fit: [30, 30]
  })
  doc.text(vineta.qrcode, 100, 100);

  const barcode = doc.openImage(`${path}/barcode.png`);
...
async function pdfGenerator(path, vineta, user, data, template){
  return new Promise((resolve) => {
    ...
    const font = `${path}/src/assets/roboto-italic.ttf`;
    const checkIMG = doc.openImage(`${path}/src/assets/check.png`);

    doc.fontSize(8);
    doc.font(font);

    [page1, page2].forEach((i, idx) => {
      const bg = doc.openImage(`${path}/src/assets/${user.id_plantilla}/${i.bg}`)
      ...
      if (idx === 0) {
        ...
        doc.fontSize(10);
        generateHeader(doc, path, vineta);
        doc.font(font);
        doc.fontSize(8);
      }
...
async function generateCodeImage(path, name, vineta, type){
  const writeFileAsync = promisify(fs.writeFile)
  const image = await generateCode(vineta, type);
  await writeFileAsync(`${path}/${name}`, image);
  return true;
}
