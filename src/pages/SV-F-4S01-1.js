...
import SVF4S011Template from "../components/templates/SV-F-4S01-1";
import '../styles/_cartas.scss';

function SVF4S011() {
  const { userId } = useParams();
  const bgStyles = {
    backgroundImage: `url(${require(`../assets/SV-F-4S01-1.png`).default})`,
  }
    ...
    <div className="container-fluid" id="cartas"> 
      <div className="text-center header d-flex flex-column" style={bgStyles}>
        <div className="layer d-flex align-items-center justify-content-center">
          <div>
            <h2 className="text-center">Carta final a mi patrocinador</h2>
            <small>Plantilla SV-F-4S01-1</small>
          </div>
        </div>
      </div>
