...
import SVC1S115Template from "../components/templates/SV-C-1S11-5";
import '../styles/_cartas.scss';

function SVC1S115() {
  const { userId } = useParams();
  const bgStyles = {
    backgroundImage: `url(${require(`../assets/SV-C-1S11-5.png`).default})`,
  }
    ...
    <div className="container-fluid" id="cartas"> 
      <div className="text-center header d-flex flex-column" style={bgStyles}>
        <div className="layer d-flex align-items-center justify-content-center">
          <div>
            <h2 className="text-center">El clima</h2>
            <small>Plantilla SV-C-1S11-5</small>
          </div>
        </div>
      </div>
