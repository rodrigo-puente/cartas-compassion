...
import templateConfig from "../../configs/SV-C-1S11-5.config";

function SVC1S115Template({ id }) {
  const TEMPLATE_ID = "SV-C-1S11-5";
  ...
  return (
    <form onSubmit={handleSubmit(id, TEMPLATE_ID, carta, form, route, img, imgs, setDisabled)}>
      <div className="form-group mb-4">
        <label htmlFor="field-1" className="mb-2 me-2">1. Ahora el clima es</label>
        <Input id="field-1" form={form} setForm={setForm} config={CONFIG} />
      </div>
      <div className="form-group mb-4">
        <label htmlFor="field-2" className="mb-2 me-2">2. Cuando el tiempo está soleado me gusta</label>
        <Input id="field-2" form={form} setForm={setForm} config={CONFIG} />
      </div>
      <div className="form-group mb-4">
        <label htmlFor="field-3" className="mb-2 me-2">3. Cuando el tiempo está lluvioso me gusta</label>
        <Input id="field-3" form={form} setForm={setForm} config={CONFIG} />
      </div>
      <div className="form-group mb-4">
        <label className="mb-2">4. Mi clima favorito es</label> 
        <div className="table-responsive">
          <table className="table mb-2">
            <thead>
              <tr>
                <th><label><input type="radio" className="form-check-input me-2" onChange={handleInputChange(form, setForm)} id="field-4-1" value="field-4-1" name="field-4" />Lluvioso</label></th>
                <th><label><input type="radio" className="form-check-input me-2" onChange={handleInputChange(form, setForm)} id="field-4-2" value="field-4-2" name="field-4" />Soleado</label></th>
                <th><label><input type="radio" className="form-check-input me-2" onChange={handleInputChange(form, setForm)} id="field-4-3" value="field-4-3" name="field-4" />Con vientos</label></th>
              </tr>
            </thead>
          </table>
        </div>
      ...
      <div className="form-group mb-4">
        <label htmlFor="field-5" className="mb-2 me-2">5. En mi comunidad el clima es bastante (cálido, caliente, helado, ...)</label>
...
export default SVC1S115Template;
